...
async function building() {
    ...
    for(let module of content) {

        // Checking if the module is a folder
        if(!fsp.lstatSync("./public/ocinfo/" + module).isDirectory()) continue;
        console.log("[INFO] Found a module named : " + module);

        // Getting the module's content
        const moduleContent = await fs.readdir(`./public/ocinfo/${module}`);

        for(let exos of moduleContent) {
            if(!fsp.lstatSync("./public/ocinfo/" + module + "/" + exos).isDirectory()) continue;
            console.log("[INFO] Found an exercice named : " + exos);

            // Getting the exos's content
            const exosContent = await fs.readdir(`./public/ocinfo/${module}/${exos}`);
            ...
            for(let file of exosContent) {
                ...
                if(file.endsWith('.html')) {
                    ...
                    console.log("[INFO][" + module + " -> " + exos + "] Found a file named " + fileName + ".html");
                    paths.push({
                        module: module,
                        exos: exos,
                        file: fileName + ".html"
